...
#' Extract and print pattern from a string along with characters that occur before and after the pattern with helpful highlighting.
#'
#' @param string The string from which to extract.
#' @param pattern Pattern to look for.
#' @param window_size Number of characters to return before and after the pattern.
...
#' @examples
#' test_string <- 'To be or not to be, that is the question.'
#' str_highlight_context(test_string, pattern = 'be', window_size = 4)
#'
str_highlight_context <- function(string, pattern, window_size){
    ...
    for(i in 1:nrow(positions_of_pattern)){
        ...
        string_extract_highlighted <- stringr::str_replace_all(string_extract, pattern, glue::glue_col("{cyan {pattern}}"))
        string_extract_all[i] <- string_extract_highlighted

    }

    return(cat(string_extract_all))
}
